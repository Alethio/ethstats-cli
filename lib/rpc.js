...
const CHECK_LAST_BLOCK_VALIDITY_INTERVAL = 300000;
...
export default class Rpc {
  ...
  connect() {
    ...
    try {
      if (!web3.currentProvider) {
        this.log.info(`Setting web3 provider to ${configRpcServer.host}:${configRpcServer.port}`);
  ...
  getBlockHashes(blockNumbers) {
    try {
      ...
      for (let i = 0; i < blockNumbers.length; i++) {
        ...
        if (blockHeader === null) {
          this.log.error(`Could not get block "${blockNumbers[i]}". Your node might be not fully synced.`);
          process.exit();
        }
  ...
  checkLastBlockValidity() {
    ...
    }, CHECK_LAST_BLOCK_VALIDITY_INTERVAL);
  }
