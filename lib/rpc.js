...
export default class Rpc {
  constructor(diContainer) {
    ...
    if (this.cli.flags.rpcHost || this.cli.flags.rpcPort) {
      this.host = (!this.cli.flags.rpcHost) ? this.config.rpcServer.host : this.cli.flags.rpcHost;
      this.port = (!this.cli.flags.rpcPort) ? this.config.rpcServer.port : this.cli.flags.rpcPort;
  ...
  connect() {
    try {
      if (!web3.currentProvider) {
        this.log.echo(`Setting web3 provider to "${this.host}:${this.port}"`);
        web3.setProvider(new web3.providers.HttpProvider(this.port ? `${this.host}:${this.port}` : this.host));
      }
  ...
  checkLatestFilter() {
    setInterval(() => {
      if (this.isConnected) {
        ...
        let nodeLastBlockNumber = web3.eth.blockNumber;
