...
export default class Server {
  constructor(diContainer) {
    ...
    this.CHECK_LAST_BLOCK_INTERVAL = 3600; // 1 hour in seconds
    this.checkLastBlockInterval = setInterval(() => {
      let clientLastBlockNumber = (this.client.lastBlock) ? this.client.lastBlock.number : null;
      if (this.lastBlockNumber !== null && this.lastBlockNumber === clientLastBlockNumber) {
        this.log.info(`No new blocks received for more than ${this.CHECK_LAST_BLOCK_INTERVAL} seconds.`);
        this.eventEmitter.emit('destroy');
      } else {
        this.lastBlockNumber = clientLastBlockNumber;
      }
