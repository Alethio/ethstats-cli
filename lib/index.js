...
const cli = new CLI(appContainer);
appContainer.cli = cli;

const socket = new Socket(appContainer);
appContainer.socket = socket;

const register = new Register(appContainer);
...
socket.client.on('open', () => {
  if (config.configStore.get('firstRun') === false) {
    if (config.configStore.get('nodeName') !== undefined && config.configStore.get('secretKey') !== undefined) {
      ...
      appContainer.logger.info(`Credentials not found. Delete config file and try again. (rm -rf ~/.config/configstore/ethstats-cli.json)`, false, true);
    }
  } else {
    if (!cli.flags.register) {
      appContainer.logger.echo(`First run detected. Please follow instructions to register your node.`);
