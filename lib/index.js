...
config.logger.showDebugs = (cli.flags.debug === undefined) ? config.logger.showDebugs : cli.flags.debug;

if (config.logger.showDebugs && !config.logger.showInfos) {
  config.logger.showInfos = true;
}

if (!config.logger.showInfos) {
  config.logger.showDateTime = false;
}
...
socket.client.on('open', () => {
  if (config.configStore.get('firstRun') === false) {
    if (config.configStore.get('nodeName') !== undefined && config.configStore.get('secretKey') !== undefined) {
      ...
      appContainer.logger.info(`Credentials not found. Delete config file and try again. (rm -rf ~/.config/configstore/ethstats-cli.json)`);
      process.exit();
    }
  } else {
    if (cli.flags.register === undefined) {
      appContainer.logger.echo(`Firt run detected. You need to provide a name for the node.`);
      socket.askNodeName();
    } else {
...
nodeCleanup((exitCode, signal) => {
  ...
  appContainer.logger.info(`Exited with code: ${exitCode}, signal: ${signal}`);
});
