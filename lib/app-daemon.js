...
pm2.connect((err) => {
  ...
  if (daemonOption === 'start') {
    pm2.start(processOptions, (error) => {
      console.log(`Ethstats daemon START ${(!error) ? chalk.green('[OK]') : chalk.red(`[FAILED] ${error.message}`)}`);
      pm2.disconnect();
    });
  }
  ...
  if (daemonOption === 'stop') {
    pm2.stop(processOptions.name, (error) => {
      console.log(`Ethstats daemon STOP ${(!error) ? chalk.green('[OK]') : chalk.red(`[FAILED] ${error.message}`)}`);
      pm2.disconnect();
    });
  }
  ...
  if (daemonOption === 'restart') {
    pm2.restart(processOptions.name, (error) => {
      console.log(`Ethstats daemon RESTART ${(!error) ? chalk.green('[OK]') : chalk.red(`[FAILED] ${error.message}`)}`);
      pm2.disconnect();
    });
  }
  ...
  if (daemonOption === 'kill') {
    pm2.killDaemon((error) => {
      console.log(`Ethstats daemon KILL ${(!error) ? chalk.green('[OK]') : chalk.red(`[FAILED] ${error.message}`)}`);
      pm2.disconnect();
      process.exit();
    });
  }
