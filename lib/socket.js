...
export default class Socket {
  constructor(appContainer) {
    ...
    this.client.on('open', () => {
      ...
      this.log.info(`Connection established with the server "${this.host}:${this.port}"`);
    });
  ...
  resolveAddNodeResponse(response) {
    if (response.success === false) {
      if (!this.cli.flags.register) {
        ...
        let counter = response.nodeName.match(/-\(\d\)$/gi);
        if (counter === null) {
          this.addNode(`${response.nodeName}-(1)`);
        } else {
          this.addNode(`${response.nodeName.replace(counter[0], '')}-(${parseInt(counter[0].replace(/[^\w\s]/gi, '')) + 1})`);
        }
      }
    } else {
