...
export default class Socket {
  ...
  askNodeName() {
    this.inquirer.prompt([
      ...
      let nodeNameRegexValidator = /^[a-zA-Z0-9-_]{3,30}$/;

      if (answer.nodeName !== undefined && answer.nodeName !== '' && nodeNameRegexValidator.test(answer.nodeName)) {
        this.addNode(answer.nodeName);
      } else {
        this.log.error(`Permitted chars: [a-zA-Z0-9-_]{3,30}`);
        this.askNodeName();
      }
    });
  }
