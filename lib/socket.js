...
export default class Socket {
  constructor(appContainer) {
    ...
    let configSocketServer = {
      host: this.config.socketServer.host,
      port: this.config.socketServer.port
    };
    let configStoreSocketServer = this.config.configStore.get('socketServer');

    if (configStoreSocketServer !== undefined) {
      configSocketServer = {
        host: configStoreSocketServer.host,
        port: configStoreSocketServer.port
      };
    }

    this.client = new PrimusSocket(`${configSocketServer.host}:${configSocketServer.port}`);
    this.client.on('open', () => {
      this.socketIsOpen = true;
      this.log.info(`Connection established with the server "${configSocketServer.host}:${configSocketServer.port}"`);
    });
